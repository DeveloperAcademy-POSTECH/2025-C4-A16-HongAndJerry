...
struct EditorTimelineView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            HStack(spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 4) { // 트랙 간 간격을 위해 spacing 추가
                    EditorRulerView()
                        .frame(height: EditConstants.rulerHeight) // 눈금자 뷰의 전체 높이
        ...
        .gesture(
            DragGesture()
                ...
                .onEnded { _ in isTimelineDragging = false }
        )
        .clipped()
    }
    ...
    private func updateOffset(_ newOffset: CGFloat) {
        ...
        if isTimelineDragging {
            let newTimeInSeconds = -clampedOffset / EditConstants.pixelsPerSecond
